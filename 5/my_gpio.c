...
static int mygpio_init(void)
{
  int err; 
     ...
     */
    /*** YOUR CODE HERE ****/
    if(gpio_request(gpio[Minor].num, gpio[Minor].label) < 0)
	printk("Error requesting GPIO %i\n",gpio[Minor].num);
 /*   
    if (gpio_request(137, "gpio137") < 0)
		printk("Error requesting GPIO %i\n", 137);
    gpio_request(gpio[0].num, gpio[0].label);
*/	
    /*
     * Set GPIO Direction
     */
    /*** YOUR CODE HERE ****/
	gpio_direction_input(130); // for one gpio
...
static void mygpio_exit(void)
{
     ...
     */
    /*** YOUR CODE HERE ****/
    gpio_free(gpio[Minor].num);
 //   gpio_free_array(gpioPort,Minor);
                 ...
                 size_t count, loff_t *f_pos)
{
   ...
   int len, result=-1, minor;
     ...
     */
    minor = MINOR(filep->f_dentry->d_inode->i_rdev);
    printk(KERN_ALERT "Reading from MyGpio [%i]\n", minor);
